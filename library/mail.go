...
func (e *Email) Bytes() ([]byte, error) {
	...
	if err := headerToBytesSeq(buff, e.Headers); err != nil {
...
func (e *Email) Attach(r io.Reader, filename string, args ...string) (a *Attachment, err error) {
	...
	filename = qEncode("utf-8", filename)
	at := &Attachment{
		Filename: filename,
		Header:   textproto.MIMEHeader{},
...
func headerToBytesSeq(w io.Writer, t textproto.MIMEHeader) (err error) {
	var seq = []string{`To`, `Cc`, `From`, `Subject`, `Disposition-Notification-To`, `MIME-Version`, `Content-Type`}
