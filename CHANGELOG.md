...
- **路径穿越漏洞修复**：路径穿越黑名单替换为白名单，增强模板操作安全性。
- **AI 配置弹窗**：增加 AI 配置弹窗，方便在 AI 对话中直接切换配置。
- **安全增强**：`GenerateRandString` 逻辑更新，提升随机字符串安全性。

## v3.6.0 `AnQiCMS` `2026-06-16`

### ✨ 新增功能

- **💬 AI Chat 智能对话**：新增 AI Chat 对话功能，支持基于 AI 大模型进行智能问答与内容创作，后台集成独立对话界面。
- **🔧 AI 工具调用（Tool Calling）**：AI 对话支持完整的工具调用链路，AI 可自主调用内置工具完成文档管理、设置修改、数据查询等操作。
- **📦 内置 AI 工具集**：新增 80+ AI 工具，涵盖文件操作、Shell 命令、Web 搜索/抓取、代码智能分析等功能，大幅扩展 AI 能力边界。
- **📄 AI 文档管理**：支持通过 AI 创建、编辑、发布、删除文档，支持分类筛选、关键词搜索、标签绑定等操作。
- **🏷️ AI 标签/分类/模型管理**：支持 AI 对标签、分类、自定义模型进行完整的增删改查操作。
- **⚙️ AI 系统管理**：支持 AI 查看和修改系统设置（站点信息、联系方式、首页TDK、安全设置、内容设置等）。
- **📊 AI 数据统计**：支持 AI 查看站点统计概览、爬虫趋势、流量趋势等数据。
- **🔗 AI SEO 工具**：支持 AI 生成 Sitemap、推送 URL、管理重定向、管理关键词库、管理锚文本。
- **👥 AI 用户与订单**：支持 AI 查看和管理用户、查询订单。
- **💡 AI 技能系统（Skill）**：新增技能管理功能，支持通过 AI 创建、查看、加载自定义技能（Skill），实现可扩展的知识体系。
- **🧩 MCP 协议支持**：集成 Model Context Protocol（MCP）标准协议，支持与外部 MCP 服务通信，为 AI 能力扩展提供标准接口。
- **📢 城市管理**：新增城市分站功能（Place）。
- **💬 留言推送**：支持将新留言推送到站点通知及外部 API 接口。

### ⚡ 功能优化

- **🌍 多语言 SEO 优化**：修复多语言 alternate 标签重复问题，优化搜索引擎对多语言站点的识别。
- **🔍 标签管理优化**：标签列表支持按分类筛选和文档关联查询，提升标签管理效率。
- **📝 AI 对话体验优化**：优化 AI 对话时的流式输出、错误处理和上下文管理。

### 🐛 问题修复

- **🔗 迁移 URL 修复**：修复从其他 CMS 迁移时 URL 转换异常的问题。
- **🏷️ 标签管理修复**：修复多语言标签处理中的若干逻辑问题。
- **📄 文档编辑修复**：修复文档编辑时某些字段未正确保存的问题。

## v3.5.9 `AnQiCMS` `2026-05-22`

### ✨ 新增功能

- **外部图片资源支持**：新增支持添加外部图片资源功能，可以引用外部 URL 图片作为网站资源使用，大幅提升图片管理的灵活性。
- **FAQPage 结构化数据**：`archiveList` 标签新增 `jsonld='faqs'` 参数，支持生成 FAQPage 结构化数据，增强 SEO 效果。

### ⚡ 功能优化

- **分类链接优化**：后台文档列表中的分类链接现可正确生成，提升管理操作体验。
- **缓存键优化**：修复文档、分类、单页、标签详情页因 token 中包含 `-` 导致缓存键不一致的问题，确保缓存正常命中。
- **LLMs 状态提示**：优化 LLMs 状态检测逻辑，状态未启用时提示更加清晰。
- **API 注释完善**：完善邮箱验证相关接口的注释说明，明确支持登录场景。
- **依赖更新**：更新项目依赖库至最新兼容版本。


## v3.5.8 `AnQiCMS` `2026-04-27`

### ✨ 新增功能

- **LLMS 支持**：新增 LLMS（大语言模型服务）集成，提供更强大的 AI 能力。
- **结构化数据支持**：增强网站 SEO 能力，支持生成结构化数据。
- **用户字段扩展**：用户信息增加 FirstName/LastName 字段，完善用户信息管理。
- **网站迁移**：新增支持从迅睿 CMS 迁移数据到 AnQiCMS。
- **文档批量操作**： 支持批量添加文档标签。

### ⚡ 功能优化

- **定时发布优化**：改进定时发布功能，提升发布稳定性和可靠性。
- **时间因子优化**：修复时间因子相关问题，确保时间相关功能正常运行。
- **多语言翻译**：优化翻译逻辑，支持按词进行纠错。
- **全文搜索引擎更新**：更新全文搜索底层引擎，提升搜索性能和准确性。

### 🐛 问题修复

- **全文搜索过滤**：修复全文搜索追加文档时未进行过滤的问题，确保搜索结果准确性。
- **静态缓存闪退**：修复开启静态缓存可能引起的程序闪退问题（[#88](https://github.com/fesiong/anqicms/issues/88)）。
- **浏览量重复计数**：修复文档浏览量重复计数的问题，确保统计准确。

## v3.5.7 `AnQiCMS` `2026-02-26`
...
- 修复开启静态缓存可能引起的程序闪退问题（[#88](https://github.com/fesiong/anqicms/issues/88)）。

## v3.5.6 `AnQiCMS` `2026-01-23`
...
- **全文搜索改进**：
  - 修复无法按内容模型筛选的问题
  - 修复搜索结果分页错误
- **参数筛选升级**：支持多选模式，可通过 `gender=男,女` 或 `gender=男&gender=女` 传参
...
- **优化** AI写作与文章采集功能，现支持**多站点同时处理**任务。
- **优化** AI写作提示词逻辑，优先采用用户自定义的要求。
- **调整** AI写作与采集文章时，关键词的使用策略改为**随机选取**，以增加内容多样性。
...
- **增强** 导航功能，现支持**自定义样式**与**配置多级导航**。
- **新增** 模板标签 `tagDataList` 对 `showFlag` 和 `showContent` 参数的支持。
...
- 新增用户 URL Token 标识功能
- 导航支持缩略图及 4 级层级结构
- 内容字段`ContentTitles` 支持扁平模式 (`list`) 和子级模式 (`children`)
- 自定义字段扩展支持文本列表、文档、分类字段类型，文本列表，图片组支持排序
- 重写锚文本替换逻辑，支持渲染时替换和选择加粗
- 文档列表标签功能增强：
  - `archiveList` 支持 `showContent` 和 `showExtra` 布尔参数
...
- 增强 `stampToDate` 功能，增加支持 format= `diff` 和 `friendly` 选项
...
- 新增 `jump`模板过滤器，跳转指定链接 `{% jump url 302 %}`

## v3.4.11 `anqicms` `2025-04-19`
...
- \[优化] 文档列表增加支持指定多分类查询
- \[优化] 文档列表调整为默认排序按 created\_time 倒叙
