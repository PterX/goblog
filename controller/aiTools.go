...
func (svc *AiChatService) getEinoTools() ([]*schema.ToolInfo, map[string]toolHandler) {
	...
	// Add site info context to system prompt
	if web != nil && web.DB != nil {
		_ = getAvailableCategories(web)
	}

	return tools, handlers
}

// getAvailableCategories returns a formatted string of available categories
func getAvailableCategories(web *provider.Website) string {
	if web == nil || web.DB == nil {
		return ""
	}
	cats, err := web.GetCategories(nil, 0, 1)
	...
	for _, c := range cats {
		...
		b.WriteString(fmt.Sprintf("%s(ID:%d)", c.Title, c.Id))
	}
	return b.String()
}
